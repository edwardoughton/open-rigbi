...
write_csv(data_aggregated, file.path(folder,'global_scenario_stats.csv'))
...
max_y_value = max(df_errorbar$mean_depth, na.rm=TRUE)

plot1 = 
  ggplot(data_aggregated, 
         aes(x=interaction, y=mean_depth, fill=continent)) + #, income_group
  geom_bar(stat="identity", position='stack') +
  geom_text(data = df_errorbar, aes(label = paste(round(mean_depth,2),"m")), 
            size = 1.5,
...
data = data %>%
  group_by(iso3, continent, climate_scenario, year, return_period) %>%
  summarize(
    mean_depth = mean(mean_depth, na.rm = TRUE),
...
plot3 = 
  ...
  geom_text(data = df_errorbar, 
    aes(label = paste(round(mean_depth,2),"m")), size = 1.5,
    vjust =-.5, hjust =.5, angle = 0) +
  theme(legend.position = 'bottom',
        axis.text.x = element_text(angle=90, hjust=1, vjust=.5, size=7)) +
  labs(colour=NULL,
...
path = file.path(folder, 'figures', 'hazard_layer_stats.png')
