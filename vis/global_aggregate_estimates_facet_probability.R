...
plot1 = ggplot(data_aggregated, 
               aes(x=year, y=mean, fill=climatescenario)) + 
...
path = file.path(folder, 'figures', 'global_coastal_flooding_facet_prob.png')
...
plot2 = 
  ...
  geom_text(aes(label = paste(round(mean,0),"Bn")), size = 1.8,
            position = position_dodge(1), vjust =.5, hjust=-.92, angle = 90)+
  theme(legend.position = 'bottom',
        axis.text.x = element_text(angle=45, hjust=1)) +
  labs(colour=NULL,
       title = "Estimated Riverine Flooding Impact to Mobile Voice/Data Cells",
  ...
  scale_fill_viridis_d(direction=1) +
  scale_x_discrete(expand = c(0, 0.15)) +
  scale_y_continuous(expand = c(0, 0), limits=c(0, max_y_value+100)) +
  facet_wrap(~probability, ncol=4, nrow=1)
