...
export = data %>%
  group_by(climatescenario, subsidence_model, year, returnperiod) %>%
  summarize(
    cell_count_baseline = sum(cell_count_baseline),
    cost_usd_baseline = sum(cost_usd_baseline),
  )
write_csv(export, file.path(folder, 'countries', paste(iso3,'_coastal.csv')))
...
max_y_value = max(data_aggregated$mean, na.rm = T)
...
plot1 = ggplot(data_aggregated, 
  ...
  geom_text(aes(label = paste(round(mean,4),"Mn")), size = 1.8,
...
plot2 = ggplot(data_aggregated, 
  ...
  geom_errorbar(data=data_aggregated, aes(y=mean, ymin=low, ymax=high),
                ...
                lwd = 0.2,
                show.legend = FALSE, width=0.7,  color="#FF0000FF") +
  geom_text(aes(label = paste(round(mean,3),"Bn")), size = 1.8,
...
path = file.path(folder, 'figures', paste(iso3, '_coastal_flooding_impacts.png'))
...
export = data %>%
  group_by(#climatescenario, subsidence_model, year, returnperiod, 
           filename) %>%
  summarize(
    cell_count_low = sum(cell_count_low),
    cost_usd_low = sum(cost_usd_low),
  )
write_csv(export, file.path(folder, 'countries', paste(iso3, '_riverine.csv')))
...
max_y_value = max(inunriver$mean, na.rm = T)
...
plot2 = 
  ...
  geom_errorbar(data=inunriver, 
                ...
                lwd = 0.2,
                show.legend = FALSE, width=0.5,  color="#FF0000FF") +
  geom_text(aes(label = paste(round(mean,4),"Bn")), size = 1.8,
...
path = file.path(folder, 'figures', paste(iso3,'_riverine_flooding_impacts.png'))
ggsave(path, units="in", width=8, height=6, dpi=300)
